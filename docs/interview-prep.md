...
- **Mon Jul 20 (AM, compressed):** Warmups 1 & 20 (both from memory, fast — Valid Parens was 20 min on Day 1, now 2 min: re-drill working). 1-D DP started: **70 Climbing Stairs** + **198 House Robber** done. Core DP ideas landed: (1) decompose by the LAST decision (`+` for counting, `max`/`min` for optimizing); (2) DP is induction — trust base cases + recurrence; (3) DP index = PREFIX subproblem boundary, not a time step. Ran late for work — **322 Coin Change pushed to Tue.**
- **Tue Jul 21 (AM, late start):** Short window — warmups + **322 Coin Change DONE**. Concept solid on first try; help was all impl debugging (array size, negative-index landmine, sentinel guard) — not a DP-understanding gap. **Mock #1 pushed to Thu Jul 23** (taking Thu off work to catch up).
- **Next up (Thu Jul 23, day off):** Mock #1 + Mock #2, behavioral prep, setup check. 2-D DP/Tries still optional/droppable (Tier 3).
...
- 1-D DP:
  - [x] [70 Climbing Stairs](https://leetcode.com/problems/climbing-stairs/) (~8 min; Fibonacci recurrence `ways(n)=ways(n-1)+ways(n-2)`. **The DP move: decompose by the LAST decision** — every path ends in a 1-step (from n-1) or 2-step (from n-2); disjoint cases → counts ADD (sum rule). `ways[k]` = count of distinct paths to k. Bottom-up O(1) space with two running vars. Same "reason about last choice" in 198, but `max` not `+`.)
  - [x] [198 House Robber](https://leetcode.com/problems/house-robber/) (~15 min; naive even/odd fails — `[2,1,1,2]` needs skipping 2 houses. `best(i) = max(skip: best(i-1), rob: nums[i] + best(i-2))`. **`max` not `+` (optimize, not count).** KEY concept: **`best[i]` = answer to the PREFIX subproblem "houses 0..i"**, NOT a time step — DP index = subproblem boundary, each cell = that subproblem's answer. `best[-1]` suffices (array is non-decreasing).) / [ ] [213 House Robber II](https://leetcode.com/problems/house-robber-ii/)
  - [x] [322 Coin Change](https://leetcode.com/problems/coin-change/) ← common (Tue Jul 21; concept was solid on FIRST try — decompose by the LAST coin, `best[a] = min(1 + best[a-c])` over coins `c <= a`, `min` because optimizing. Help was all impl debugging, not concept: (1) array size = `amount+1` not `amount`; (2) loop must be `range(1, amount+1)` to reach the target; (3) the LANDMINE — no guard on `c <= a` means `a-c` goes negative and Python silently indexes from the END (garbage); (4) `inf → -1` guard at return; (5) `amount==0` returns 0 not -1. Single-coin amounts fall out FREE: `best[c] = 1 + best[0] = 1`. Infinity sentinel POISONS unreachable amounts — traced `[2],3 → best[3]=1+best[1]=1+inf=inf → -1`.)
